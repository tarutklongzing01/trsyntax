...
import { Ban, Check, Copy, Gift, KeyRound, Link2, LoaderCircle, Monitor, Pencil, Plus, RotateCcw, Search, Trash2 } from 'lucide-react';
...
function generateLicenseKey(type: 'trial' | 'full' = 'full') {
  const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  const bytes = crypto.getRandomValues(new Uint8Array(12));
  const body = Array.from(bytes, (byte) => alphabet[byte % alphabet.length]).join('');
  return `${type === 'trial' ? 'TRIAL' : 'TRSYN'}-${body.slice(0, 4)}-${body.slice(4, 8)}-${body.slice(8, 12)}`;
}
...
export function HwidManager() {
  ...
  const [createType, setCreateType] = useState<'trial' | 'full'>('full');
  ...
  function startCreate(type: 'trial' | 'full' = 'full') {
    setCreateType(type);
    setForm({ licenseKey: generateLicenseKey(type), customerName: '', email: '', programId: visiblePrograms[0]?.id || '', duration: type === 'trial' ? '7d' : '15d' });
    setMessage('');
    setOpen(true);
  }
  ...
  async function submit(event: FormEvent) {
    ...
    if (createType === 'trial' && licenses.some((license) => license.licenseType === 'trial' && license.programId === form.programId && license.email.toLowerCase() === normalizedEmail)) {
      setError('อีเมลนี้เคยได้รับ Trial สำหรับโปรแกรมนี้แล้ว');
      return;
    }
    setBusy('create'); setError('');
    try {
      await createHwidLicense({ ...form, email: normalizedEmail, licenseType: createType, programName: program.name, expiresAt: expiryFromDuration(form.duration) ?? null });
      setOpen(false); setMessage(createType === 'trial' ? 'สร้าง Trial Key 7 วันเรียบร้อยแล้ว' : 'เพิ่มไลเซนส์เรียบร้อยแล้ว');
  ...
  return <>
    <header className="hwid-header"><div><p>LICENSE MANAGEMENT</p><h1>จัดการ HWID</h1><small>สร้างไลเซนส์ ผูกเครื่อง และควบคุมสิทธิ์การใช้งาน</small></div><div className="hwid-header-actions"><Button variant="outline" onClick={() => startCreate('trial')}><Gift/> สร้าง Trial 7 วัน</Button><Button className="admin-add" onClick={() => startCreate('full')}><Plus/> เพิ่มไลเซนส์</Button></div></header>
    ...
    <section className="admin-panel hwid-panel">
      ...
      <div className="table-scroll"><table className="hwid-table"><thead><tr><th>LICENSE KEY</th><th>ประเภท</th><th>ลูกค้า / โปรแกรม</th><th>เครื่องที่ผูก</th><th>หมดอายุ</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody>
        {loading && <tr><td colSpan={7} className="hwid-empty"><LoaderCircle className="spin"/> กำลังโหลดไลเซนส์...</td></tr>}
        {!loading && filtered.length === 0 && <tr><td colSpan={7} className="hwid-empty">ยังไม่มีไลเซนส์ที่ตรงกับตัวกรอง</td></tr>}
        {filtered.map((license) => {
          const isExpired = expired(license); const isBusy = busy === license.id;
          return <tr key={license.id}><td><button className="license-key-copy" onClick={() => void navigator.clipboard.writeText(license.licenseKey)} title="คัดลอก License Key"><code>{license.licenseKey}</code><Copy/></button></td><td><span className={`license-type ${license.licenseType}`}>{license.licenseType === 'trial' ? 'TRIAL' : 'FULL'}</span></td><td><strong>{license.customerName}</strong><small>{license.email || 'ไม่มีอีเมล'} · {license.programName}</small></td><td>{license.hwid ? <code className="hwid-code"><Link2/>{license.hwid}</code> : <span className="unlinked">ยังไม่ผูกเครื่อง</span>}</td><td>{license.expiresAt ? new Date(license.expiresAt).toLocaleDateString('th-TH') : 'ไม่หมดอายุ'}</td><td><span className={`hwid-status ${isExpired ? 'expired' : license.status}`}>{isExpired ? 'หมดอายุ' : license.status === 'active' ? 'เปิดใช้งาน' : 'ปิดใช้งาน'}</span></td><td><div className="hwid-actions"><Button variant="outline" size="sm" disabled={isBusy || !license.hwid} onClick={() => void run(license.id, () => resetHwidLicense(license.id), 'รีเซ็ตเครื่องเรียบร้อยแล้ว')}><RotateCcw/> รีเซ็ต</Button><Button variant="outline" size="sm" disabled={isBusy} onClick={() => startEdit(license)}><Pencil/> แก้ไข</Button><Button variant="outline" size="sm" disabled={isBusy} onClick={() => void run(license.id, () => setHwidLicenseStatus(license.id, license.status === 'active' ? 'disabled' : 'active'), 'อัปเดตสถานะเรียบร้อยแล้ว')}>{license.status === 'active' ? <Ban/> : <Check/>}{license.status === 'active' ? 'ปิด' : 'เปิด'}</Button><Button variant="destructive" size="icon-sm" disabled={isBusy} onClick={() => setPendingDelete(license)} aria-label={`ลบไลเซนส์ ${license.licenseKey}`}><Trash2/></Button></div></td></tr>;
        })}
      </tbody></table></div>
    </section>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="hwid-dialog"><DialogHeader><DialogTitle>{createType === 'trial' ? 'สร้าง Trial Key ฟรี' : 'เพิ่มไลเซนส์ใหม่'}</DialogTitle><DialogDescription>{createType === 'trial' ? 'ทดลองใช้งาน 7 วัน ผูกได้หนึ่งเครื่อง และรับได้ครั้งเดียวต่อโปรแกรม' : 'สร้างคีย์สำหรับผูกกับเครื่องของลูกค้า'}</DialogDescription></DialogHeader><form onSubmit={submit} className="hwid-form"><label>License Key<Input required value={form.licenseKey} onChange={(event) => setForm({ ...form, licenseKey: event.target.value.trim().toUpperCase() })}/></label><div><label>ชื่อลูกค้า<Input required value={form.customerName} onChange={(event) => setForm({ ...form, customerName: event.target.value })} placeholder="ชื่อผู้ใช้งาน"/></label><label>อีเมล<Input required type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="name@example.com"/></label></div><div><label>โปรแกรม<NativeSelect value={form.programId} onChange={(event) => setForm({ ...form, programId: event.target.value })}>{visiblePrograms.map((program) => <NativeSelectOption value={program.id} key={program.id}>{program.name}</NativeSelectOption>)}</NativeSelect></label><label>อายุไลเซนส์{createType === 'trial' ? <Input value="7 วัน (ทดลอง)" disabled/> : <DurationSelect value={form.duration} onChange={(duration) => setForm({ ...form, duration })}/>}</label></div><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>ยกเลิก</Button><Button type="submit" disabled={busy === 'create'}>{busy === 'create' ? <LoaderCircle className="spin"/> : createType === 'trial' ? <Gift/> : <Plus/>} {createType === 'trial' ? 'สร้าง Trial Key' : 'สร้างไลเซนส์'}</Button></DialogFooter></form></DialogContent></Dialog>
